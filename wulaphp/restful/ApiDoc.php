...
class ApiDoc {
     ...
     * @return array
     */
    public static function doc(string $api, string $v, string $method = 'get'): array {
        $apis = explode('.', $api);
        if (count($apis) != 3) {
    ...
    private static function scanVers(array &$ms, string $path, string $id) {
        $data = [];
    ...
    private static function scanApis(array &$ms, string $path, string $id) {
        $data = [];
        $dir  = new \DirectoryIterator($path);
        $ids  = explode('/', $id);
        $idx  = $ids[0];
